...
%% remove border
imwrite(im_gnd,'out_img_org.bmp');
imwrite(im_l,'out_img_low.bmp');
imwrite(im_b,'out_img_bic.bmp');
imwrite(im_h,'out_img_cnn.bmp');
...
figure(1), imshow(im_b); title('Bicubic Interpolation');
figure(2), imshow(im_h); title('SRCNN Reconstruction');
%% show results
